...
class NAT (object):
  ...
  def _append_Actions_Tx_External(self, msg, packet, event, port):
    # todo: send out ARP request to get server IP instead of using our static arp table
    msg.actions.append(of.ofp_action_dl_addr.set_src(self.external_mac))
    ...
    if ipp.srcip.in_network(self.internal_network):
      ...
      if (tcpp.ACK):
        ...
        if (self.nat_ports_in_use[nat_port] == _TCP_STATE_HANDSHAKE):
          ...
          log.debug("TCP connection ESTABLISHED. nat_ports_in_use[%d] = %d" % (nat_port, self.nat_ports_in_use[nat_port]))
          ...
          msg = self._append_Actions_Tx_External(msg, packet, event, self.client_to_nat_port[(ipp.srcip, tcpp.srcport)])
          self.connection.send(msg)
          log.debug("Installed flow mod for incoming INTERNAL packets")
      ...
      # if we're in connected state then we need to install reverse
      # flow modification rules, i.e. for packets coming from external network
      elif (self.nat_ports_in_use[nat_port] == _TCP_STATE_CONNECTED):
  ...
  def _handle_PacketIn (self, event):
    ...
    if packet.dst.is_multicast:
      ...
      if icmpp:
        ...
        raise Exception("ERROR: UNHANDLED message TYPE")
